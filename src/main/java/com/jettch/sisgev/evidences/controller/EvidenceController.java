...
import com.jettch.sisgev.evidences.dto.AssociateSegmentRequest;
...
import com.jettch.sisgev.evidences.dto.RejectEvidenceRequest;
...
public class EvidenceController {
    ...
    /**
     * Upload multipart de evidência (BE-14). Campos do mobile (§10.4):
    ...
    public EvidenceResponse approve(@PathVariable UUID id) {
        return service.approve(id);
    }

    @PatchMapping("/{id}/reject")
    public EvidenceResponse reject(@PathVariable UUID id, @Valid @RequestBody RejectEvidenceRequest request) {
        return service.reject(id, request.reason());
    }

    @PatchMapping("/{id}/mark-duplicated")
    public EvidenceResponse markDuplicated(@PathVariable UUID id) {
        return service.markDuplicated(id);
    }

    @PatchMapping("/{id}/associate-segment")
    public EvidenceResponse associateSegment(@PathVariable UUID id, @Valid @RequestBody AssociateSegmentRequest request) {
        return service.associateSegment(id, request.roadSegmentId());
    }
}
